...
public sealed class VendorCategoryConfiguration : IEntityTypeConfiguration<VendorCategory>
{
    public void Configure(EntityTypeBuilder<VendorCategory> builder)
    {
        ...
        builder.HasIndex(x => new { x.CompanyId, x.CategoryCode })
            .IsUnique()
            .HasDatabaseName("uq_vendor_category_company_id_category_code");

        builder.HasIndex(x => new { x.CompanyId, x.CategoryName })
            .HasDatabaseName("ix_vendor_category_company_id_category_name");

        builder.HasIndex(x => x.CompanyId)
            .HasDatabaseName("ix_vendor_category_company_id");

        builder.HasIndex(x => x.StatusId)
            .HasDatabaseName("ix_vendor_category_status_id");
